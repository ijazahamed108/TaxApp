...
const Dashboard = () => {
  ...
  const handleConfirmPayment = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:5000api/computetax/paytaxes', {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000api/auth/user-info', {
  ...
  const fetchSubmissionHistory = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000api/computetax/get', {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
  ...
  const handleComputeTax = async () => {
    try {
      const response = await axios.post('http://localhost:5000api/computetax/store', taxFormData, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
