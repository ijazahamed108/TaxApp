...
const AdminDashboard = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://localhost:5000api/admin/users', {
  ...
  const fetchTaxAuditRecords = async () => {
    try {
      const response = await axios.get('http://localhost:5000api/admin/get-audit-records', {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleUpdateAuditRecord = async () => {
    try {
      await axios.put(
        `http://localhost:5000api/admin/update-tax-audit-record/${selectedAuditRecordId}`,
        selectedAuditRecord,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        }
  ...
  const handleUpdateUser = async () => {
    try {
      await axios.put(`http://localhost:5000api/admin/update-user/${selectedUserId}`, selectedUser, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
      fetchUsers(); 
      setSelectedUserId('');
